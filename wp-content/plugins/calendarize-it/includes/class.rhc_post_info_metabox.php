...
class rhc_post_info_metabox {
	...
	function rhc_post_info_metabox($post_type,$capability){
		...
		$this->post_meta_boxes = new post_meta_boxes(array(
			...
			'styles'=>array(),
			'scripts'=>array('postinfo-metabox'),
	...
	function postinfo_postmeta_exclude($arr){
		return array_merge($arr,array(
			'extra_info_size', 'fc_click_link','fc_exdate', 'fc_rdate', 'enable_postinfo','enable_featuredimage','enable_venuebox','enable_postinfo_image','rhc_top_image', 'rhc_dbox_image', 'rhc_tooltip_image'
		));
	}
	...
	function wp_ajax_update_postinfo(){
		...
		$out = do_shortcode('[post_info frontend="0" id="'.$postinfo_boxes_id.'"]');
...
<div id="post_extrainfo_quick_icons" class="post_extrainfo_quick_icons">
	<ul class="quick_icons_tabs_nav">
		<?php include 'quick_icons.php';//defines $quick_icons?>
	...
	<select id="postinfo_boxes_id" class="postinfo_box_select">
		<?php include 'postinfo_boxes.php';//defines $postinfo_boxes?>
...
//					$options[$field]=sprintf('%s(%s)',$label,substr($value,0,10));
		...
		if(count($options)==0){
			require_once RHC_PATH.'includes/meta_fields_default_labels.php';
			foreach(array('fc_start','fc_start_time','fc_end','fc_end_time','fc_start_datetime','fc_end_datetime') as $field){
				$label = isset($default_meta_field_labels[$field])?$default_meta_field_labels[$field]:$field;
				$options[$field]=$label;
			}
		}
