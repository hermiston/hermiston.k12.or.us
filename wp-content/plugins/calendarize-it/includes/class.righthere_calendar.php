...
class righthere_calendar {
	...
	function init($install=false){
		...
		register_post_type( RHC_EVENTS, array(
			...
			'taxonomies' => array(),
			'menu_icon'=> RHC_URL.'css/images/rhc.png'			
		));
...
<label class="rhc_filter_label rhc_opt_filter"><?php _e('Venue','rhc')?></label>:&nbsp;<?php wp_dropdown_categories( array('class'=>'rhc_opt_filter','selected'=>(isset($_REQUEST['f_rhc_venue'])?$_REQUEST['f_rhc_venue']:0), 'name'=>'f_rhc_venue','id'=>'f_rhc_venue', 'show_option_all'=> sprintf('--%s--',__('show all','rhc')),'taxonomy'=>RHC_VENUE) );?>
<?php endif;?>
<?php if(isset($this->taxonomies[RHC_CALENDAR])):?>
<label class="rhc_filter_label rhc_opt_filter"><?php _e('Calendar','rhc')?></label>:&nbsp;<?php wp_dropdown_categories( array('class'=>'rhc_opt_filter','selected'=>(isset($_REQUEST['f_rhc_calendar'])?$_REQUEST['f_rhc_calendar']:0), 'name'=>'f_rhc_calendar','id'=>'f_rhc_calendar', 'show_option_all'=> sprintf('--%s--',__('show all','rhc')),'taxonomy'=>RHC_CALENDAR) );?>
<?php endif;?>
<?php if(isset($this->taxonomies[RHC_ORGANIZER])):?>
<label class="rhc_filter_label rhc_opt_filter"><?php _e('Organizer','rhc')?></label>:&nbsp;<?php wp_dropdown_categories( array('class'=>'rhc_opt_filter','selected'=>(isset($_REQUEST['f_rhc_organizer'])?$_REQUEST['f_rhc_organizer']:0), 'name'=>'f_rhc_organizer','id'=>'f_rhc_organizer', 'show_option_all'=> sprintf('--%s--',__('show all','rhc')),'taxonomy'=>RHC_ORGANIZER) );?>
<?php endif;?>
<?php		
		}
	}	
	...
	function pre_get_posts($query){
		if(is_admin()&&$query->query['post_type']==RHC_EVENTS){
