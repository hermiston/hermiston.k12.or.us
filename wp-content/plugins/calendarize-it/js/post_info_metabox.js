jQuery(document).ready(function($){
	...
	$('.pinfo_quick_icon').on('click',function(e){
		var me = this;
		var fields = ['post_extrainfo_type','post_extrainfo_taxonomy','post_extrainfo_taxonomymeta','post_extrainfo_postmeta','post_extrainfo_label','post_extrainfo_value'];
...
function init_sortable(){
	jQuery(document).ready(function($){
			...
			$(inp).sortable({
				...
				'receive': function(e,ui){			
					...
					$('.rhc-extra-info-cell').each(function(i,inp){	
						var col = $(this).parents('.fe-maincol').attr('rel');
		...
		$('.rhc-extra-info-cell').each(function(i,inp){
			...
			$('.post_extrainfo_control .post_extrainfo_cell').each(function(j,control){
				...
				if( 'post_extrainfo_taxonomymeta'==_input_id ){
					_value = post_extrainfo[index].taxonomymeta_field + '|' + post_extrainfo[index].taxonomymeta;
				}
				//--			
				_row
		...
		$('.rhc-extra-info-cell .pinfo-save').on('click',function(e){
			...
			var column = $(this).parents('.fe-maincol').attr('rel');
			var p = $(this).parents('.rhc-extra-info-cell');
			var _type = p.find('.post_extrainfo_type').val(); 
			var arr = p.find('.post_extrainfo_taxonomymeta').val().split('|');
			var _taxonomymeta_field = '';
			var _taxonomymeta = '';
			if(arr.length==2){
				_taxonomymeta_field = arr[0];
