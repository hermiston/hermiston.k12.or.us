<?php
/*
Plugin Name: HSD Network Latest Posts
 ...
 * **** Patch for the hsd_nlp_custom_excerpt function
 *
 * -- Greggo
...
require_once dirname( __FILE__ ) . '/hsd-network-latest-posts-widget.php';
...
function hsd_network_latest_posts( $parameters ) {
    global $wpdb;
    ...
    if ( ! preg_match( "/,/", $post_ignore ) ) {
        $post_ignore = array( 0 => (int) $post_ignore );
    // if the user passes more than one value separated by commas
    } else {
        // create an array
    ...
    if ( isset( $blogs ) ) {
        ...
        if( empty( $all_posts ) ) {
            ...
            echo '<div class="alert">' . __( "Sorry, I couldn't find any recent posts matching your parameters.", "trans-nlp" ) . '</div>';
            return;
        }
        // Sort by date (regardless blog IDs)
        if ( 'true' == $sort_by_date ) {
        ...
        echo '<div class="custom-sidebar gdl-divider recentpost-widget nlp-instance-' . $instance . '">';
        ...
        if ( $paginate && $posts_per_page ) {
                    ...
                    if ( 'false' == $title_only ) {
                        ...
                        if( $display_content != 'true' ) {
                            ...
                            if( $auto_excerpt != 'true' ) {
                                ...
                                echo hsd_nlp_custom_excerpt($excerpt_length, $field->post_excerpt, $all_permalinks[$field->guid],$excerpt_trail);
                            // Extract excerpt from content
                            } else {
                                // Get the excerpt
                                echo hsd_nlp_custom_excerpt($excerpt_length, $field->post_content, $all_permalinks[$field->guid],$excerpt_trail);
                            }
                        } else {
            ...
            echo '<div class="nlposts-ulist-pagination pagination">';
            ...
            foreach ( $all_posts as $field ) {
                ...
                if ( 'true' == $thumbnail ) {
                    ...
                    echo '<div class="recent-post-widget-context">
                          ...
                          <a href="' . $all_permalinks[$field->guid] . '">' . $field->post_title . '</a>
                          </div>';
...
add_shortcode('hsd_nlposts','hsd_network_latest_posts_shortcode');
 ...
 */
function hsd_nlp_custom_excerpt($count, $content, $permalink, $excerpt_trail){
 ...
 * return @html_tags
 */
function hsd_nlp_display_type($display_type, $instance, $wrapper_list_css, $wrapper_block_css) {
    // Instances
...
function hsd_network_latest_posts_init() {
    global $wp_locale;
...
add_action( 'widgets_init', create_function( '', '
    /* Check RTL
     * This function cannot be called from the hsd_network_latest_posts_init function
    ...
    if( is_rtl() ) {
        ...
        wp_register_style( "hsd-nlpcss-rtl", plugins_url("/css/default_style-rtl.css", __FILE__) );
        // Load the style
        wp_enqueue_style( "hsd-nlpcss-rtl" );
 ...
 */
function hsd_network_latest_posts_uninstall() {
    // Delete widget options
    delete_option( 'widget_hsd_nlposts_widget' );
    // Delete the shortcode hook
    remove_shortcode( 'hsd_nlposts' );
}
...
function hsd_nlp_shortcode_button() {
    if ( current_user_can( 'edit_posts' ) && current_user_can( 'edit_pages' ) ) {
        add_filter( 'mce_external_plugins', 'hsd_nlp_shortcode_plugin' );
        add_filter( 'mce_buttons', 'hsd_nlp_register_button' );
    }
}
// Hook the button into the TinyMCE editor
function hsd_nlp_register_button( $buttons ) {
    array_push( $buttons, "|" , "hsd_nlposts" );
    return $buttons;
}
// Load the TinyMCE NLposts shortcode plugin
function hsd_nlp_shortcode_plugin( $plugin_array ) {
...
// Languages
add_action( 'plugins_loaded', 'hsd_nlp_load_languages');
