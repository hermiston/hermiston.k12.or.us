...
class Media_Rename {
	...
	function print_js() {
		if ($this->is_media_rename_page) {
			wp_enqueue_script('media-rename', plugins_url('js/scripts.js?rename=' . __('Rename') . '&rename_retitle=' . urlencode(__('Rename & Retitle')), __FILE__), array('jquery'));
		}
	}
	...
	static function do_rename($attachment_id, $new_filename, $retitle = 0) {
		...
		wp_update_attachment_metadata($attachment_id, $new_sizes_data = wp_generate_attachment_metadata($attachment_id, $new_file_abs_path));
