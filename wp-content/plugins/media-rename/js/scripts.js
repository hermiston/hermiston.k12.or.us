(function($) {
	...
	var script_src = $('script').eq( $('script').length-1 ).attr('src');
	var options = {
		rename: script_src.match(/rename=([^&]+)/)[1],
		rename_retitle: script_src.match(/rename_retitle=([^&]+)/)[1]
	};
		...
		if (!is_media_single) {
			$('.tablenav select[name^=action]').each(function() {
				for (option in options) {
					$('option:last', this).before( $('<option>').attr('value', option).text( decodeURIComponent(options[option].replace(/\+/g, '%20')) ) );
				}
			});
		}
